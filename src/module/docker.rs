...
mod tests {
    use super::*;
    use crate::core::{Config, DockerConfig, RegistryMapping};
    use std::collections::HashMap;

    fn create_test_config() -> Config {
        let mut registries = HashMap::new();
        registries.insert("docker.io".to_string(), RegistryMapping {
            upstream: "registry-1.docker.io".to_string(),
            auth_host: "auth.docker.io/token".to_string(),
            _auth_type: "docker".to_string(),
            enabled: true,
        });
        registries.insert("ghcr.io".to_string(), RegistryMapping {
            upstream: "ghcr.io".to_string(),
            auth_host: "ghcr.io/token".to_string(),
            _auth_type: "github".to_string(),
            enabled: true,
        });
    ...
    fn test_docker_matches() {
        ...
        assert!(provider.matches("v2/library/ubuntu/manifests/latest"));
        assert!(provider.matches("token?service=registry.docker.io"));
    }

    #[test]
    fn test_docker_transform_v2() {
        let provider = DockerProvider::new();
        let config = create_test_config();

        let path = "v2/library/ubuntu/manifests/latest".to_string();
        let result = provider.transform(path, &config);
        assert_eq!(result, "https://registry-1.docker.io/v2/library/ubuntu/manifests/latest");

        let path = "v2/ghcr.io/user/repo/manifests/latest".to_string();
        let result = provider.transform(path, &config);
        assert_eq!(result, "https://ghcr.io/v2/user/repo/manifests/latest");
    }

    #[test]
    fn test_docker_transform_token() {
        let provider = DockerProvider::new();
        let config = create_test_config();

        let path = "token?service=ghcr.io&scope=repository:user/repo:pull".to_string();
        let result = provider.transform(path, &config);
        assert!(result.starts_with("https://ghcr.io/token?"));
        assert!(result.contains("service=ghcr.io"));
    }
}
